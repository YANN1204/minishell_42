...
/*   Updated: 2024/02/19 17:22:39 by yrio             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (1)
	{
		...
		if (args_split[0])
		{
			...
			if (!ft_strncmp(args_split[0], "env", 3) && \
				ft_strlen(args_split[0]) == ft_strlen("env"))
				ft_env(&minishell);
			if (!ft_strncmp(args_split[0], "ls", 2) && \
				(ft_strlen(args_split[0]) - 1 == ft_strlen("ls") ||
				ft_strlen(args_split[0]) == ft_strlen("ls")))
				ls_cmd();
			if (!ft_strncmp(args_split[0], "pwd", 3) && \
				ft_strlen(args_split[0]) == ft_strlen("pwd"))
				ft_pwd();
			...
			if (!ft_strncmp(args_split[0], "unset", 5) && \
				...
				ft_strlen(args_split[0]) - 1 == ft_strlen("unset")))
